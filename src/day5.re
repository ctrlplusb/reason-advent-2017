/*
 ...
 next one. Start at the first instruction in the list.
 ...
 */
let rec loop = (instructions, idx, count) => {
  let nextIdx = idx + instructions[idx];
  if (nextIdx < 0 || nextIdx + 1 > Array.length(instructions)) {
    count
  } else {
    instructions[idx] = instructions[idx] + 1;
    loop(instructions, nextIdx, count + 1)
  }
};

let stepsToExit = (instructions) =>
  switch instructions {
  | [||] => (-1)
  | _ => loop(instructions, 0, 1)
  };