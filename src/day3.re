...
type state = (int, direction, int, int);

let rec loop = (state: state) =>
  switch state {
  | (1, _, x, y) => abs(x) + abs(y)
  | (id, Right, x, y) => loop((id - 1, x + 1 > y ? Up : Right, x + 1, y))
  | (id, Up, x, y) => loop((id - 1, y - 1 == x * (-1) ? Left : Up, x, y - 1))
  | (id, Left, x, y) => loop((id - 1, x - 1 == y ? Down : Left, x - 1, y))
  | (id, Down, x, y) => loop((id - 1, y + 1 == x * (-1) ? Right : Down, x, y + 1))
  };

let distance = (id) => loop((id, Right, 0, 0));